...
function isPlayingSound(szSound){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        if(s_aSounds[szSound].playing()){
            return true;
        }else{
            return false;
        }
...
function fadeStopSound(szSound,iDuration){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        s_aSounds[szSound].once("fade",function(){
            stopSound(szSound);
        });
        s_aSounds[szSound].fade(s_aSounds[szSound].volume(),0,iDuration);
    }
}
